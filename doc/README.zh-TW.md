...
### 功能
本工具提供了一套實用程式來管理大型影片收藏：

- **影片轉碼器 (Video Encoder)**：使用 FFmpeg 批次將影片轉換為 HEVC/x265 格式，在保持畫質的同時節省空間。
- **重複檔案檢查器 (Duplication Checker)**：使用 BLAKE3 雜湊演算法識別重複檔案，並將它們移動到獨立目錄以清理儲存空間。
- **縮圖目錄生成器 (Contact Sheet Generator)**：自動為影片生成預覽縮圖目錄。利用場景偵測選取具代表性的時間點，並平行處理縮圖以提升速度。
- **依類型自動移動 (Auto Move by Type)**：掃描目錄並根據副檔名將檔案整理到子資料夾中。
- **孤立檔案移動器 (Orphan File Mover)**：偵測並重新安置「孤立」檔案——例如不再有對應影片檔的附屬檔案或縮圖。
- **影片重新命名器 (Video Renamer)**：依據影片時長排序並重新命名檔案，以保持特定的順序。
