...
mod tests {
    ...
    fn create_test_mover() -> OrphanFileMover {
        let shutdown_signal = Arc::new(AtomicBool::new(false));
        OrphanFileMover::new(shutdown_signal)
    }
    ...
    fn test_scan_and_group() {
        ...
        let groups = mover.scan_and_group(base_path).unwrap();
    ...
    fn test_move_orphan_files() {
        ...
        let mover = create_test_mover();
        let groups = mover.scan_and_group(base_path).unwrap();
        let result = mover.move_orphan_files(&groups, base_path).unwrap();
    ...
    fn test_custom_orphan_folder_name() {
        ...
        let mover = create_test_mover().with_orphan_folder_name("moved_files");
        let groups = mover.scan_and_group(base_path).unwrap();
        let result = mover.move_orphan_files(&groups, base_path).unwrap();
