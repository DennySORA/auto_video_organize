...
use crate::tools::{
    FileGroup, OrphanFileMover as OrphanFileMoverTool, OrphanMoveResult, validate_directory_exists,
};
use anyhow::Result;
use console::style;
...
impl OrphanFileMover {
    ...
    pub fn run(&self) -> Result<()> {
        ...
        // 建立移動器
        let mover = OrphanFileMoverTool::new(Arc::clone(&self.shutdown_signal));

        // 掃描並分組
        println!("{}", style("掃描檔案中...").dim());
        let groups = mover.scan_and_group(&directory)?;
        ...
        let result = mover.move_orphan_files(&groups, &directory)?;
    ...
    fn print_group_summary(&self, groups: &[FileGroup]) {
        let orphan_files = OrphanFileMoverTool::get_orphan_files(groups);
        let paired_groups = OrphanFileMoverTool::get_paired_groups(groups);
